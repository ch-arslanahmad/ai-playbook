...
OpenCode (Claude etc.) or your agent keeps changing its mind, even though, a decision wa already made. It suggests repeating the same discussion every session, which is frustrating and inefficient.
...
So agent know what you already chose & why.
...
A decision log gives it memory with a reason attached. So your starter stack is now simple. `AGENT.md` for rules, project_brief for the target, and `DECISION.md` for trade-offs. 
